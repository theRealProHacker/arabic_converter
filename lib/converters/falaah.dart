...
import 'package:arabic_converter/converters/shared.dart';
...
extension on Map<String, String> {
  Map<String, String> get swapKeys {
    return {for (final pair in entries) pair.value: pair.key};
  }
}
...
final _arabAliases = <String, String>{
  for (final vowel in _allLatinVowels) vowel.toUpperCase(): vowel.x2
};
...
var _arabCharsMap = _sorted({
  ...
  ..._arabChars,
  for (final pair in _arabChars.entries) pair.key.x2: pair.value + shaddah,
...
final _arabExtrasMap = _sorted({
  for (final pair in arabVowels.entries) ...{
...
final _latinNums = arabNums.swapKeys;

final _latinMap = {..._arabCharsMap.swapKeys, ..._arabExtrasMap.swapKeys};

final Set<String> _acceptedLatinChars = {..._latinMap.values, "_"};

String Function(String) _handler(String Function(String) next, String split) {
  return (text) {
    if (text.isEmpty) return "";
    return text.split(split).where((token) => token.isNotEmpty).map(next).toList().join(split);
  };
}
...
final handleLatinText = _handler(_handleLatinLine, "\n");
...
String _latinWordToArab(String token) {
  ...
  // Ignores
  for (final ignore in arabIgnores) {
  ...
  token.split("").asMap().forEach((i, value) {
    for (final pair in arabVowels.entries) {
      if (pair.value.short == value) {
        if (i == 0 ||
            arabVowels.values.any((vowel) => vowel.short == newToken[i - 1]) ||
            arabAppends.containsValue(token[i - 1])) {
  ...
  token = token.replaceAll(
      "أِ", // This break has a reason. If you don't believe my then try
      "إ\u0650");

  // error raising
  final error = token
      .split("")
      .where((char) => !_acceptedArabChars.contains(char))
      .join("");
...
final handleArabText = _handler(_handleArabLine, "\n");
...
String _handleArabToken(String token) =>
    _arabNumToLatin(token) ?? arabWordToLatin(token);
...
String arabWordToLatin(String token) {
  const escapeBeforeh = {"d", "s", "t", "k", "g"};
  const couldBeVowel = <String, String>{"y": "ii", "w": "uu"};

  final beforeNotVowel = {_allLatinVowels, "°"};
  ...
  token.split("").asMap().forEach((i, arab) {
    ...
    if (latin != null) {
      if (i > 0) {
        ...
        if (latin == "h" && escapeBeforeh.contains(newToken[i - 1]) ||
            latin == "H" && newToken[i - 1] == "D") {
          latin = "_" + latin;
          // Convert semiconsonants to their vowel part
        } else if (couldBeVowel.containsKey(latin) &&
            !beforeNotVowel.contains(newToken[i - 1])) {
  ...
  final error = token
      .split("")
      .where((char) => !_acceptedLatinChars.contains(char))
      .join("");
  if (error.isNotEmpty) throw ParseError(error);
